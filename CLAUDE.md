...
>
> **안전장치:** 닫으려는 워크트리에 커밋 안 한 변경이 있으면 **닫지 않습니다** — 자동 닫기(post-merge)는 해당 워크트리를 건너뛰고, 수동 실행은 중단하며 커밋/stash를 안내합니다. 워크트리 제거는 깨끗할 때만 일어나므로 `--force`를 쓰지 않습니다(스크립트가 미커밋 변경을 삭제할 수 없음).
...
- 모든 커밋은 pre-commit 훅(시크릿 스캔 + 컴파일 + 테스트)을 통과해야 함
- 최종 머지는 사람이 직접 승인 후 처리

### 검사 계층 원칙 (commit vs PR)
보안 검사는 **두 층**으로 나눈다 — 한쪽이 다른 쪽을 대체하지 않는다.
- **commit (자동·기계 판단):** 시크릿 스캔·컴파일·테스트. 매 커밋 즉시. 비밀값(API 키·비밀번호·서비스계정 키)은 한 번 커밋되면 히스토리에 영구 잔존하므로 **커밋 시점에 자동 차단**한다(`.githooks/secret-scan.sh`).
- **PR (사람·판단):** 로직 정확성·아키텍처·변경 전체 검토. 사람 리뷰를 commit 시점으로 옮기지 않는다(자기검토 독립성 없음, 소단위 흐름 붕괴).
- 시크릿 스캔 오탐 시 의도적 우회: `git commit --no-verify` (예외 상황만).
...
| 센서 (계산형) | pre-commit: 시크릿 스캔(`.githooks/secret-scan.sh`) — 비밀값 유출 차단 | 커밋마다 |
