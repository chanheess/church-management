...
public class RepresentativePrayerService {
  ...
  private List<RepresentativePrayerScheduleRow> buildScheduleRowsWithConfig(
      int year, RepresentativePrayerConfig cfg) throws IOException {
    ...
    for (LocalDate sun : sundays) {
      ...
      rows.add(
          new RepresentativePrayerScheduleRow(
              iso,
              label,
  ...
  private RepresentativePrayerYearView buildYearView(
      List<RepresentativePrayerScheduleRow> rows, RepresentativePrayerConfig cfg) {
    ...
    for (int m = 1; m <= 12; m++) {
      ...
      for (RepresentativePrayerScheduleRow r : byMonth.get(m)) {
        ...
        days.add(
            new RepresentativePrayerTableDay(
                r.getIsoDate(),
                d.getDayOfMonth() + "일",
                r.getLabel(),
                r.getResolvedLeader(),
  ...
  /**
   * 주보 '대표기도·헌금위원' 표에 쓸, 해당 월의 모든 주일별 담당 (스왑·날짜 예외 반영). {@link
   * com.saeanyang.management.service.ExcelReaderService} 의 월별 주일 나열과 같은 순서다.
   */
  ...
  private Map<LocalDate, String> buildBaseSchedule(
      int year, List<String> rotation, Map<String, String> dateOverrides) {
  ...
  private Map<LocalDate, String> applyNameOverrides(
      Map<LocalDate, String> resolved, Map<String, String> nameOverrides) {
    if (nameOverrides == null || nameOverrides.isEmpty()) return resolved;
    Map<LocalDate, String> map = new LinkedHashMap<>(resolved);
    nameOverrides.forEach(
        (key, value) -> {
          try {
  ...
  private RepresentativePrayerConfig loadConfigRaw() throws IOException {
    ...
    RepresentativePrayerConfig cfg =
        objectMapper.readValue(bytes, RepresentativePrayerConfig.class);
