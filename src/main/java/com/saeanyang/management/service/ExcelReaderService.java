...
public class ExcelReaderService {
  ...
  /** 오늘을 기준으로 "이번 주의 주일(일요일)" 날짜를 계산한다. - 오늘이 일요일이면 오늘 날짜 - 그 외 요일이면, 이번 주의 다가오는 일요일 날짜 */
    ...
    for (Person person : people) {
      ...
      try {
        ...
        if (!birthThisYear.isBefore(weekStart) && !birthThisYear.isAfter(weekEnd)) {
          ...
          System.out.println(
              "  → 생일자 발견: " + person.getName() + " (생일: " + month + "월 " + day + "일)");
        }
      } catch (Exception e) {
        System.out.println("  → 생일 파싱 실패: " + person.getName() + ", 생일값: " + birthdayStr);
      }
  ...
  private List<String> parsePositions(String positionStr) {
    ...
    return Arrays.stream(positionStr.split("[,/]"))
        .map(String::trim)
        .filter(s -> !s.isEmpty())
        .filter(s -> s.contains("간사") || s.contains("목자") || s.contains("리더") || s.contains("인턴"))
  ...
  /** 엑셀에서 연락처 및 출석현황 데이터를 읽어온다. 셀별로 그룹화하여 반환한다. */
