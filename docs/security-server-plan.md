...
서비스는 관리자만 접근 가능한 구조로 전환하고, 로그인 보호, 관리자 2FA, 개인정보 AES 암호화, 접근 로그, 다운로드/출력 로그를 1차 보안 범위로 둔다.
...
- 관리자 로그인은 비밀번호만으로 끝내지 않고 2FA를 포함한다.
- 신뢰기기에서는 정책 기간 동안 2FA를 생략할 수 있어야 한다.
...
- 관리자 계정에는 2FA를 적용한다.
- 신뢰기기 기반 정책을 사용한다.
- 새 기기 또는 신뢰 기간이 만료된 기기에서는 2FA를 요구한다.
- 2FA 성공 후 관리자가 신뢰기기로 등록할 수 있다.
...
- 해제되지 않은 신뢰기기 토큰만 중복 방지한다.
  - 예: `UNIQUE INDEX ON admin_trusted_devices(token_hash) WHERE revoked_at IS NULL`
...
- 로그인, 로그아웃, 인증 실패, 2FA, 신뢰기기 이벤트 로그를 추가한다.
...
- 새 기기 로그인 시 2FA가 요구되는지 확인한다.
- 신뢰기기 등록 후 정책 기간 동안 2FA가 생략되는지 확인한다.
- 신뢰기기 만료 또는 해제 후 다시 2FA가 요구되는지 확인한다.
...
- 관리자 2FA 방식: 이메일/문자/인증 앱/패스키 중 신뢰기기 등록 전 추가 인증 수단 선택 필요.
- 신뢰기기 유지 기간: 예를 들어 7일, 14일, 30일 중 운영 정책 선택 필요.
